...
class Model_LstmFc_v2(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        out_fc2_hat = F.log_softmax(out_fc2_hat,dim = -1)
        return out_fc2_hat
        #sys.exit()
...
class Model_LstmFc_v3(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn = nn.LSTM(256, 512, num_layers=2, batch_first=True, bidirectional=True) # input_dim, hidden_dim
    ...
    def forward(self,x):
        ...
        out, (h, cell) = self.rnn(x) # (batch_size, seq, input_size)
        if print_flag :
          print("Shape output from  RNN is ", out.shape) # [B, 10, 1024]
